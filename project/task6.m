...
for i = 1:length(k)
    [M_g, dt_1] = advection(0, k(i));
    [M_gl, dt_2] = advection(1, k(i));
    ...
    hold on
    plot(d_g*dt_1, 'r.', 'DisplayName', 'Gauss Stability')
    hold on
    plot(d_gl*dt_2, 'g .', 'DisplayName', 'Gauss Lobatto Stability')
    xlabel("real")
    ylabel("imag")
    title(['Scaled Eigenvalue , k = ' num2str(k(i))])
    grid on;
    ...
    hold on
    plot(w,'color','b', 'DisplayName', 'RK4 Stability')
    legend('FontSize', 16, 'Location','west')
    hold off
end

% 1D advection test program
function [Matrix, Delta_t] = advection(quad_type, k)
...
if quad_type == 0
    ...
    [pg,wg] = get_gauss_lobatto_quadrature(k+1); 
end
