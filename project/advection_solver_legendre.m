...
[values,derivatives] = evaluate_legendre_basis(k, pg);
face_values = evaluate_legendre_basis(k, [-1;1]);
...
% set initial condition: as opposed to the Lagrange program we need to do a
% projection to the Legendre coefficients (we could also use the Vandermonde
% matrix from the Gauss-Lobatto points)
u=zeros(kp1*n,NT+1);
for e=1:n
    u(e*kp1-k:e*kp1,1) = values'\ ... 
        analytical(y(2*e-1)+(y(2*e)-y(2*e-1))*(0.5+0.5*pg), 0);
end
...
uu_0 = zeros(size(xx));
uu = zeros(size(xx));
val = evaluate_legendre_basis(k, xx_unit);
for e=1:n
...
values_err = evaluate_legendre_basis(k, pg_err);
for e=1:n
